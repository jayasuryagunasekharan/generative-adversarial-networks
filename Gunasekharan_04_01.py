...
def build_generator():
    ...
    # Third Conv2DTranspose layer with tanh activation
...
class DCGAN(tf.keras.Model):
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            combined_images = tf.concat([real_output, fake_output], axis=0)
            labels = tf.concat([tf.zeros((batch_size, 1)), tf.ones((batch_size, 1))], axis=0)
            labels += 0.55 * tf.random.uniform(labels.shape)

            # Compute discriminator loss
            d_loss = self.loss_fn(labels[:batch_size], combined_images[:batch_size])

            # Compute generator loss
            g_loss = self.loss_fn(tf.ones_like(labels[:batch_size]), fake_output)
...
def train_dcgan_mnist():
    ...
    # train the DCGAN
    dcgan.fit(x_train, epochs=50, batch_size=32, callbacks=callbacks, shuffle=True)
